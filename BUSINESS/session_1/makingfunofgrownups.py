...
class PreFunc():
    ...
    def pideColores(self):
        for i in range(2):
            self.colores[f"color_{i+1}"] = str(input(f"Introduce el color {i+1}: "))

    def muestraColores(self):
        for i in range(2):
            print(f"Color {i+1}: {self.colores[f'color_{i+1}']}")

    def mezclaColores(self):
        color1 = self.colores.get("color_1", "").lower()
        color2 = self.colores.get("color_2", "").lower()
    ...
    def menu(self):
        ...
        self.__lista = ["Cuadrado", "Rectángulo", "Círculo", "Triángulo", "Trapecio"]
        for i, figura in enumerate(self.__lista, 1):
            print(f"{i}. {figura}")
...
class Problems():
    ...
    def problem1(self):
        self.prefunc.pideColores()
        self.prefunc.muestraColores()
        self.prefunc.mezclaColores()
